...
if df is not None:
    ...
    # --- Diagnostics: Column sanity checks & summaries ---
    with st.expander("Diagnostics: inspect factor columns (sanity checks)"):
        if df is not None and allocation_cols:
            ...
            for col in allocation_cols:
                ...
                nan_pct = 0.0 if n_total == 0 else (n_total - n_valid) / n_total
                # Detect if values look like raw returns (centered near 0 with negatives) instead of factors (>0)
                looks_like_returns = (non_null.min() < 0) or (non_null.max() < 0.5)
                # Basic stats in factor space
                vmin = float(non_null.min()) if n_valid else float("nan")
                vmax = float(non_null.max()) if n_valid else float("nan")
                # Geometric mean and vol (using log factors)
                if n_valid:
                    logf = np.log(non_null)
                    geo_mean = float(np.exp(logf.mean()))
                    log_std = float(logf.std(ddof=1)) if n_valid > 1 else float("nan")
                else:
                    geo_mean = float("nan"); log_std = float("nan")
                diag_rows.append({
                    "Column": col,
                    "valid_rows": n_valid,
                    "nan_%": round(nan_pct * 100, 2),
                    "min": round(vmin, 6) if n_valid else None,
                    "max": round(vmax, 6) if n_valid else None,
                    "geomean_factor": round(geo_mean, 6) if n_valid else None,
                    "log_factor_std": round(log_std, 6) if n_valid else None,
                    "looks_like_returns_not_factors": bool(looks_like_returns),
                })
            import pandas as pd as _pd_alias  # alias to avoid overshadowing
            diag_df = _pd_alias.DataFrame(diag_rows)
            st.dataframe(diag_df, use_container_width=True)
            # Simple monotonicity check: higher equity should typically have higher vol of log factors
            try:
                order_for_check = ["LBM 100E","LBM 90E","LBM 80E","LBM 70E","LBM 60E","LBM 50E","LBM 40E","LBM 30E","LBM 20E","LBM 10E","LBM 100F"]
                check = diag_df.set_index("Column").reindex(order_for_check)["log_factor_std"].dropna()
                if not check.is_monotonic_decreasing:
                    st.warning("Volatility by allocation (std of log factors) is not strictly decreasing with lower equity. This can indicate column mix‑ups or unit issues.")
            except Exception:
                pass
            # Specific highlight for LBM 20E
            if "LBM 20E" in df.columns:
                s20 = df["LBM 20E"].dropna()
                if len(s20):
                    st.write("LBM 20E preview (first 10, last 10):")
                    st.write(_pd_alias.concat([s20.head(10), s20.tail(10)]))
                    if (s20 < 0).any():
                        st.error("LBM 20E contains negative values; these look like returns, not factors. Convert to (1+return).")
...
# Choose data frequency (sets row increment)
freq_choice = st.radio(
    "Data frequency in the factors worksheet:",
    ["Annual (one row per year)", "Monthly (12 rows per year)"],
    index=0,
    help="Pick 'Annual' if each row is already a one‑year factor. Pick 'Monthly' if rows are monthly factors and you want one year steps.")
row_increment = 1 if freq_choice.startswith("Annual") else 12
...
if st.button("Calculate"):
    if df is None:
        ...
        # Cross‑check: show quick table of geo means & vol we computed earlier
        if 'diag_df' in locals():
            show = diag_df.copy()
            show['Pretty'] = show['Column'].map(allocation_mapping).fillna(show['Column'])
            st.caption('Factor column summary (for quick plausibility scan):')
            st.dataframe(show[['Pretty','valid_rows','nan_%','min','max','geomean_factor','log_factor_std','looks_like_returns_not_factors']], use_container_width=True)
        ...
        st.info("If your worksheet already contains **annual factors**, set frequency to **Annual (one row per year)** above. Use **Monthly** only if rows are monthly factors and you want to step one year at a time.")
