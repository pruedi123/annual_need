...
with col4:
    freq_choice = st.radio(
        ...
        index=0,
        help="If each row is already a yearly factor, choose Annual. If rows are monthly factors and you want to step one year at a time, choose Monthly.")
    ...
    if df is None or not allocation_cols:
        ...
        if not plot_df.empty:
            ...
            fig = go.Figure(data=[go.Bar(
                ...
                marker_color=colors,
                text=plot_df['Required Annual Investment'],
                textposition='outside'
            )])
            fig.update_layout(title="Required Annual Investment by Allocation",
